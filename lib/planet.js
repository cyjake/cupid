...
function byPublishedDesc(a, b) {
  const aTime = (a.updated || a.published).getTime()
  const bTime = (b.updated || b.published).getTime()
...
class Planet {
  constructor(root) {
    ...
    this.feeds = data.feeds.map(feed => {
      return new Feed(feed.uri, {
        author: feed.author,
        postSelector: feed.postSelector
      })
    })
  ...
  * parse({ count, timeout }) {
    ...
    this.posts = this.feeds
      ...
      .filter(post => (post.updated || post.published))
  ...
  * write(dest = 'target', { skipView } = { skipView: false }) {
    ...
    for (const file of files) {
      if (['.html', '.xml'].includes(path.extname(file))) {
        if (!skipView) yield this.writeView(file, dest)
