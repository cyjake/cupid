...
class Post {
  ...
  static fromItem(item) {
    ...
    if (item['content:encoded']) data.content = item['content:encoded']
...
class Feed {
  constructor(uri, { author, postSelector }) {
    this.uri = uri
    this.author = author || {}
  ...
  rectify() {
    ...
    for (const post of this.posts) {
      const $ = cheerio.load(`<div>${post.content}</div>`, { decodeEntities: false })
      const $el = $('div').first()
  ...
  parseRSS(rss) {
    ...
    for (const name of ['title', 'description', 'content:encoded', 'link']) {
  ...
  * parse({ timeout }) {
    ...
    if (this.postSelector) {
      yield this.posts.map(post => {
        return fetch(post.uri, { timeout: timeout })
          .then(res => {
            const $ = cheerio.load(res.text, { decodeEntities: false })
            post.content = $(this.postSelector).html()
          })
      })
    }
